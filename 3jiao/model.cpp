...
void Objmodel::calculateNormal() {
    ...
    for (int i = 0; i < f_num; i++) {
        for (int j = 0; j < 3; j++) {
            v1[j] = vertex[face_vertex[i][10] - 1][j] - vertex[face_vertex[i][0] - 1][j];
            v2[j] = vertex[face_vertex[i][11] - 1][j] - vertex[face_vertex[i][12] - 1][j];
        }
        //叉乘1
        f_norm1 = vectorCross(v1, v2);
        v1 = v2;
        for (int j = 0; j < 3; j++) {
            v2[j] = vertex[face_vertex[i][0] - 1][j] - vertex[face_vertex[i][13] - 1][j];
        }
        //叉乘2
        f_norm2 = vectorCross(v1, v2);
        v1 = v2;
        for (int j = 0; j < 3; j++) {
            v2[j] = vertex[face_vertex[i][14] - 1][j] - vertex[face_vertex[i][0] - 1][j];
        }
        //叉乘3
        ...
        //rawNormal[face_vertex[i][15] - 1] = f_norm1;
        //rawNormal[face_vertex[i][16] - 1] = f_norm2;
...
void Objmodel::readFile(string path) {
    ...
    while (!infile.eof()) {
        ...
        /*a1 = { 1,2,3 };
        vertex.push_back(a1);
        cout << vertex[0][0] << vertex[0][17] << vertex[0][18];*/
...
void Objmodel::showObj(GLint mode1, GLint mode2) {
    ...
    {
        //glNormal3f(vertex_normal[face_normal[i][0] - 1][0], vertex_normal[face_normal[i][0] - 1][19], vertex_normal[face_normal[i][0] - 1][20]);
        glNormal3f(vertex_normal[face_vertex[i][0] - 1][0], vertex_normal[face_vertex[i][0] - 1][21], vertex_normal[face_vertex[i][0] - 1][22]);
        glVertex3f(vertex[face_vertex[i][0] - 1][0], vertex[face_vertex[i][0] - 1][23], vertex[face_vertex[i][0] - 1][24]);

        glNormal3f(vertex_normal[face_vertex[i][1] - 1][0], vertex_normal[face_vertex[i][26] - 1][27], vertex_normal[face_vertex[i][28] - 1][29]);
        glVertex3f(vertex[face_vertex[i][1] - 1][0], vertex[face_vertex[i][31] - 1][32], vertex[face_vertex[i][33] - 1][34]);

        glNormal3f(vertex_normal[face_vertex[i][2] - 1][0], vertex_normal[face_vertex[i][36] - 1][37], vertex_normal[face_vertex[i][38] - 1][39]);
        glVertex3f(vertex[face_vertex[i][2] - 1][0], vertex[face_vertex[i][41] - 1][42], vertex[face_vertex[i][43] - 1][44]);
    }
    glEnd();
}
...
vector<GLfloat> Objmodel::getCenter(/*GLfloat &x,GLfloat &y,GLfloat &z*/) {
    ...
    y_max = y_min = vertex[0][45];
    z_max = z_min = vertex[0][46];*/
    ...
    /*for (int i = 1; i < v_num; i++) {     //这是单独去最大值与最小值来得出包围盒
        ...
        if (y_max < vertex[i][49])
            y_max = vertex[i][50];
        if (y_min > vertex[i][51])
            y_min = vertex[i][52];

        if (z_max < vertex[i][53])
            z_max = vertex[i][54];
        if (z_min > vertex[i][55])
            z_min = vertex[i][56];
    }*/
